...
echo [4/8] Resetting the development database...
docker compose exec app php bin/console doctrine:database:drop --force --if-exists
docker compose exec app php bin/console doctrine:database:create
docker compose exec app php bin/console doctrine:migrations:migrate -n

echo [5/8] Clearing the application cache...
docker compose exec app php bin/console cache:clear

echo [6/8] Building Tailwind CSS...
docker compose exec app php bin/console tailwind:build

echo [7/8] Preparing the test database and running the test suite...
...
echo [8/8] Running static analysis (PHPStan)...
docker compose exec app vendor/bin/phpstan analyse -c phpstan.neon.dist --memory-limit=1G

echo [9/9] Starting the background worker for URL click tracking...
