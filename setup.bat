...
echo [4/7] Clearing the application cache...
docker compose exec app php bin/console cache:clear

echo [5/8] Building Tailwind CSS...
docker compose exec app php bin/console tailwind:build

echo [6/8] Preparing the test database and running the test suite...
...
echo [7/8] Running static analysis (PHPStan)...
docker compose exec app vendor/bin/phpstan analyse -c phpstan.neon.dist --memory-limit=1G

echo [8/8] Starting the background worker for URL click tracking...
